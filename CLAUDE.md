...
- **Account limit**: `generateSale` deducts the net on-account amount (`amount - change`) from `accountLimit.current` in the same transaction as the sale. It does **not** cap the tender at the available limit — On Account records a debt, so a customer with no limit left (or none set at all) can still take goods and settle later; `accountLimit.current` goes negative and the sale lands `PENDING`. Store Credit is the opposite and is still hard-checked, since prepaid value can't go negative.
...
- **Store credit is the single customer wallet**: `storeCredit.current` is fed by refunds, manual issuance (`adjustStoreCredit`) and change the customer chose to keep at checkout. It is a spendable tender in Process Sale (`NEXT_PUBLIC_STORE_CREDIT_ID`), validated against the available balance by `generateSale`/`updateSale`, and handed back by `voidSale`. There is deliberately no second wallet — one pot keeps the balance reconcilable.
- **Kept change**: when a sale has change and a customer, the Pay sheet asks before ringing up. Answering yes sets `changeToStoreCredit`, which makes `generateSale` record `changeAmount: 0`, a `netAmount` equal to the full tender and `changeCreditedAmount`, then credit `storeCredit`. The cash stays in the drawer, so the shift tally still expects it. `voidSale` reverses both directions in one signed adjustment (credit spent comes back, credit created from kept change goes away). A sale whose change was kept can no longer be edited (`assertSaleIsEditable`) — void it instead.
