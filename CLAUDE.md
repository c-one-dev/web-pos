...
- **Two customer wallets, one source each**: `storeCredit.current` is fed by refunds and manual issuance (`adjustStoreCredit`); `currentBalance.current` is fed _only_ by change the customer chose to leave on their account at checkout. Both are spendable tenders in Process Sale (`NEXT_PUBLIC_STORE_CREDIT_ID` / `NEXT_PUBLIC_CURRENT_BALANCE_ID`), both are validated against the available balance by `generateSale`/`updateSale`, and both are handed back by `voidSale`. One source per wallet is what makes them reconcilable — do not credit store credit from kept change, or vice versa.
- **Kept change**: when a sale has change and a customer, the Pay sheet asks before ringing up. Answering yes sets `changeToStoreCredit` (the flag name predates Current Balance), which makes `generateSale` record `changeAmount: 0`, a `netAmount` equal to the full tender and `changeCreditedAmount`, then credit `currentBalance`. The cash stays in the drawer, so the shift tally still expects it. A sale whose change was kept can no longer be edited (`assertSaleIsEditable`) — void it instead.
